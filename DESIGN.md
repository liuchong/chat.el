...
│                    Persona Engine                               │
├─────────────────────────────────────────────────────────────────┤
│                    File Operations Layer                        │  <- ⭐ 新增
│         (文件读取、搜索、裁剪、拼接、修改、格式化)                  │
├─────────────────────────────────────────────────────────────────┤
...
### 4.4 文件操作层 (chat-files.el) ⭐新增

文件操作层为 AI Agent 提供统一的文件访问接口，支持读取、搜索、修改、格式化等操作。

#### 4.4.1 核心功能分类

**基础操作**:
- `chat-files-read` - 读取文件内容（支持偏移量和限制）
- `chat-files-read-lines` - 读取指定行范围
- `chat-files-exists-p` - 检查文件存在性
- `chat-files-list` - 列出目录内容

**文件移动与组织**:
- `chat-files-move` - 移动/重命名
- `chat-files-copy` - 复制文件
- `chat-files-delete` - 删除文件/目录
- `chat-files-mkdir` - 创建目录
- `chat-files-batch-rename` - 批量重命名

**搜索功能**:
- `chat-files-grep` - 文件内搜索（支持上下文）
- `chat-files-find` - 跨文件搜索

**内容提取与裁剪**:
- `chat-files-extract` - 按位置提取内容
- `chat-files-head/tail` - 读取文件头/尾
- `chat-files-slice` - 提取多段不连续内容
- `chat-files-truncate` - 截断文件到指定行数

**内容修改**:
- `chat-files-write` - 写入/追加内容
- `chat-files-replace` - 文本替换
- `chat-files-insert-at` - 在指定位置插入
- `chat-files-patch` - 原子性多补丁应用

**文件拼接与分割**:
- `chat-files-concat` - 多文件合并
- `chat-files-split` - 按行数分割文件

**扩展功能**:
- `chat-files-diff` - 文件对比
- `chat-files-checksum` - 计算文件哈希
- `chat-files-encode` - 编码转换
- `chat-files-backup/restore` - 备份与恢复
- `chat-files-rotate` - 日志轮转
- `chat-files-format` - 自动格式化（JSON/XML/Elisp）
- `chat-files-preview` - 生成文件预览（含大纲）
...
```elisp
;; 读取文件前 100 行
(chat-files-read "~/project/main.py" 0 5000)

;; 搜索并显示上下文
(chat-files-grep "def " "~/project/main.py" 3)

;; 批量替换
(chat-files-patch "~/config.el"
                  '((:search "old-value" :replace "new-value" :line 42)))

;; 生成文件预览（含结构大纲）
(chat-files-preview "~/project/main.py" 100)
```
```
