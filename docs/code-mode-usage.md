...
3. 输入斜线命令前缀时，输入区附近会显示有限数量的纯提示；提示不获得焦点，也不能选择
4. RET 始终直接发送或执行当前输入一次，不会先把某个提示填入输入区
5. 显式按 TAB 时，命令只补到唯一候选或候选的最长公共前缀，不启动选择界面
6. 只有显式按 TAB 且当前 token 看起来像路径时，才计算并补全文件名
7. 需要多行提示词时，用 S-RET 插入换行，不会直接发送
```
