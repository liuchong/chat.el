     ...
     ((memq 'programming_plan_create ids)
      (mapconcat
       ...
       '("Programming plan boundaries:"
         ...
         "- For exactly one bounded mutation through `files_write`, `files_replace`, or `files_patch`, call `programming_plan_skip` with reason `single-bounded-action` and that exact `tool_name` instead of creating a TODO plan. The audited skip is available only once per task and permits one matching mutation plus targeted verification. If that edit fails or another mutation becomes necessary, create a durable TODO plan; never issue another skip or use a skip to hide multi-step work."
