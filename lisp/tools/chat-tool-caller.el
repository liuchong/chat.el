...
(defun chat-tool-caller--tool-usage-guidance ()
  ...
  (mapconcat
   ...
   '("Tool usage guidance:"
     ...
     "- `shell_execute` is for lightweight readonly inspection when file tools are not enough, and for intentional `sleep N` delays that must block this tool call."
     ...
     "- `programming_compile_task` / `work_task_start` return immediately. To wait for that task, call `programming_task_output` / `work_task_output` with `wait_seconds`. Never start a background `sleep` to wait."
     ...
     ((memq 'programming_plan_create ids)
      (mapconcat
       ...
       '("Programming plan boundaries:"
         "- Answer-only and read-only work needs no plan. For substantial coding or more than one bounded mutation, call the directly available `programming_plan_create` before the first action; ordinary coding creates the durable TODO list and starts its first dependency-ready item atomically."
