  ...
  (let ((resolved-model (chat-coding-eval--model-name provider model-name)))
    (lambda (task workspace done)
      (let* ((session
           ...
           (approvals 0)
           (stale-writes 0)
           (stagnation-detections 0)
           (stagnation-recoveries 0)
           (stagnation-stops 0)
           (run nil)
         ...
         (let* ((steps (if (null steps)
           ...
           (list
            ...
            `((provider . ,(symbol-name provider))
              ...
              (stagnationDetectionCount . ,stagnation-detections)
              (stagnationRecoveryCount . ,stagnation-recoveries)
              (stagnationStopCount . ,stagnation-stops)
        ...
        (funcall
         ...
         (lambda (event)
           ...
           (let ((type (plist-get event :type))
             ...
             (pcase type
               ('stagnation-detected (cl-incf stagnation-detections))
               ('stagnation-recovered (cl-incf stagnation-recoveries))
               ('stagnation-stopped (cl-incf stagnation-stops))))
