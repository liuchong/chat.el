    ...
    (mapconcat
     ...
     (delq nil
           (list
            ...
            "- A tool call may be ruled on by an automatic approval policy rather than by the user. A denial from it says so, and gives its reason: it means this route is closed, not that the user objected and not that the task is over."
            "- After such a denial, take a different route that stays within the reason given, or finish and say which part was not permitted. Do not send the same call again unchanged, and do not ask the user to approve it."
