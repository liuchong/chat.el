...
## 步骤耗时与终态标记（2026-09-03 补充）

每步的墙钟耗时与整轮的终态是转录的一部分，不是转瞬即逝的状态：

- 每个折叠行显示它那一段的耗时：从上一个 part 的时间戳到本段最后一个
  时间戳的差值。第一段之前没有可比的点，不显示。耗时随折叠行持久可见，
  重开、折叠、重绘后不变。
- 一轮结束（完成/停止/取消/出错）写入一条 `turn-outcome` 类别的持久
  消息：终态、总耗时（从请求创建到终态）、步数。该类别永不折叠、永不
  回灌给模型（属 display-only）。状态栏在请求结束后显示该结果，下一个
  请求开始时清除。
- 进行中的实时叙述行附带已耗时秒数；状态栏终态段只在空闲时显示。
- 发送路径的每个准备阶段（上下文与工具提示词准备、agent 启动前）都写入
  请求诊断轨迹并立即反映到实时叙述。一条 Lisp 死循环拦得住定时器，
  拦不住已经落盘的轨迹——卡死时请求面板与状态行指向最后一个完成的
  阶段，这就是护栏能做到的全部。
