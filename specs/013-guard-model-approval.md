...
项目构建、测试和静态检查同样可以进入 A 层,但必须同时满足以下条件:工具只能是具有
强制隔离边界的 `programming_compile_task`;执行目录解析后必须位于项目根内;命令必须是
单一 shell 段,不能包含重定向、命令替换、后台执行或命令串联;解析后的 argv 必须命中
明确维护的 build/test/lint 入口及其检查形态。识别层覆盖验收语料实际使用的 Emacs ERT、
Go、Rust、Python、JavaScript/TypeScript、Zig、Clojure、Java、C/C++ 与通用构建工具,
但不把同样的命令授权给普通 shell 工具。任意解释器代码、无法识别的子命令、复合命令、
越界目录和边界不明的工具仍进入 C 层。该快路引用与 C 层相同的 build/test/lint ALLOW
