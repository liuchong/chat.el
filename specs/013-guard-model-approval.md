...
3. **判决要允许弃权。** arXiv 2608.17994《Judge, Retrieve, or Abstain》的做法是让
   ...
   但"弃权 = 拒绝"这条要有(第 5 条)。
4. **影子运行作为调优设施保留,但不作为默认。** Vercel AI SDK 有现成形态:
   `shadow(approval, opts)` 把策略照常求值、把裁决报给 `onDecision`,但**告诉 SDK
   无论策略说什么都算批准**
   (<https://github.com/vercel/ai/tree/main/packages/policy-opa>)。这个设施我们要
   (第 12 条),因为它是把提示词调准的唯一手段。但 AgentEval 那句 "keep it
   shadow-only until it clears the promotion bar" 我们不照搬成默认值:默认开影子等于
   `guarded` 出厂即不生效,而它的兜底恰好是那套不可用的查表规则,用户开了发现照旧被拒,
   下一步就是去开 `dangerous`——安全上更差。默认让 guard 真的说话,影子留给调优。
...
另外提供一个**影子运行**开关(第 12 条)用于调优:guard 照常裁决与记录但不生效。
默认关——`guarded` 出厂即由 guard 真正裁决。
...
### 12. 影子运行:一个调优设施,不是一个模式

**先把名字理清,因为容易混。** 审批模式只有三个:`manual`、`guarded`、`dangerous`
(第 1 条)。"自动审批 / 自动裁决 / guard 模式"说的都是 `guarded`——guard 真的裁决,
它说放行就放行,说拒绝就拒绝。

**影子运行不是第四个模式,而是 `guarded` 内部的一个开关。**`chat-approval-guard-shadow`
为真时,guard 照常发请求、照常裁决、照常记录,但**裁决不生效**:放行与否退回没有
guard 时的既有规则。也就是说 guard 在旁边判、在旁边记,但不做决定。

它存在的唯一目的是**调优**:提示词第一版必然不准,而把它调准需要"guard 会怎么判"与
"实际发生了什么"的成对样本。用自造测试集调不出来,因为我们还不知道真实分布长什么样。
现成形态是 Vercel AI SDK 的 `shadow(approval, opts)`。

**默认关。**`guarded` 出厂即由 guard 真正裁决。理由是默认开影子会让这个模式出厂即
不生效,而它的兜底恰好是那套不可用的查表规则——用户开了 `guarded` 发现照旧被拒,
下一步就是去开 `dangerous`,安全上比让 guard 生效更差。第六节那条"先影子直到过线"
的业界建议在这里不适用,因为那条建议假设兜底策略本身是可用的。

影子运行下要能导出成对样本(工具、参数摘要、guard 的 `decision` / `matched-rule` /
`reason` / `confidence`、既有规则的结果、以及最终是否执行),供离线比对。非影子运行
时同样记录裁决(第 8 条),两者的区别只在"是否由它决定",不在"是否记录"。
...
28. 为真时:guard 照常发请求、照常记录,但**放行与否由既有规则决定**。构造一个
    guard 会拒、既有规则会放的调用,断言它执行了,且事件里记着 guard 本来会拒。
29. 反向:构造一个 guard 会放、既有规则会拒的调用,断言影子下它**没有**执行,
    而关掉影子后同一调用会执行。
29a. 影子日志可导出成对样本,含 guard 裁决与实际结果两侧。
29b. 非影子运行时裁决同样落日志(区别只在是否由它决定)。
