...
**1.4** **十七种**事件类型全部落盘,一种不漏。写这份 spec 时按注释里的清单算了十五种,
施工时从 `chat-agent--emit` 的调用点重数是十七种——清单本身就漏了两种,这正说明覆盖率
必须由测试从代码里数出来,不能由人抄注释。新增事件类型时必须同时给出它的 payload 形状,
由测试断言覆盖率——**没有兜底分支**,因为兜底分支就是这次事故的成因。
