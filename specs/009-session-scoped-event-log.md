...
**1.1** 每个会话有且仅有一份 wire 文件:`~/.chat/sessions/wire/<id>.jsonl`。

放在**独立子目录**而不是与 context 文件同目录并列命名。原本设计的
`<id>.wire.jsonl` 在施工时被 `chat-session-list` 否掉了:它按 `*.jsonl` 通配、取
`file-name-base` 当 session id,所以同目录下的任何 `.jsonl` 都会在每次列会话时被当作
会话交给 `chat-session-load`。子目录让这件事在结构上不可能发生,而不是靠一条要维护的
过滤规则。索引文件同理放在会话目录**之外**(`~/.chat/index.jsonl`),否则重建索引的那
一次遍历会把索引本身当成会话读进来。

**1.1a** 追加用 `write-region ... t`,不用 context 流的「整份复制再改名」。
`chat-session--atomic-append-jsonl` 每追加一条就把整个文件复制一遍,是 O(记录数²);
context 流每轮追加几条还能忍,事件流一轮就有几十条,不能走同一条路。
...
**1.4** `chat-agent--emit` 实际发出的全部事件类型必须落盘,一种不漏。事件总数由测试
从代码读取并与投影分支比较,不得在 spec 或测试里维护一份会过期的手写计数。新增事件
类型时必须同时给出它的 payload 形状,由测试断言覆盖率——**没有兜底分支**,因为兜底
分支就是这次事故的成因。

**1.4a** 投影函数对任何形状的事件都必须是全函数,包括 run 为空、字段缺失的事件。
观察者一旦有能力抛错,就有能力把一次本来跑得通的 run 变成一次「被观察时失败」的 run。
...
施工结论:信封自带 `timestamp_ms`,所以 `turn-start` 与该轮第一条 `stream-chunk` 的时间
差**就是**首字节延迟,不需要再单独埋一个字段。这条因此按「由信封时间戳可导出」满足,
不新增计时管线。
...
**2.4a** 每次真实模型请求在 transport 启动后记录 `model-request-started`，payload 至少
包含实际 `provider`、具体 `model` 和 `request_id`。该记录来自模型 runtime 实际发出的
started event，不得由 campaign 描述、UI 选择或 readiness 请求代替。它是判断一次 Run
是否发生模型身份漂移的事实来源。

**本条未实现,已确认是缺口而不是遗漏。** 施工时核实:厂商返回的 `usage` 目前在整个
代码库里没有任何一处被读取。OpenAI 兼容的流式接口默认不返回 usage,要在请求里带
`stream_options.include_usage`,各厂商的字段名与返回时机又不一致——这是一条独立的传输层
改动,不该塞进这次落盘的施工里。落盘侧已经就位:`usage` 一旦被解析出来,记一条
`kind:"usage"` 即可,不需要再改本流的任何结构。
...
施工选择:把 `chat-log` 本身改成宏,展开成
`(when chat-log-enabled (chat-log--write …))`,一次性覆盖全部调用点,而不是逐个改。
前提是仓库不带 `.elc`、且没有任何地方把 `chat-log` 当函数值传递(施工前核实过,只有两处
`funcall 'chat-log`,都是本次新写的,已改回直接调用)。
...
- 一次带工具调用、带 steering、带压缩、带取消的会话跑完后,只读 wire 文件就能
  ...
  压缩发生在哪里、token 花了多少
- 代码实际发出的每一种事件类型都有投影测试,且断言 payload 里没有 `:run`
- 一次两轮 Agent Run 产生两条 `model-request-started`，每条都带实际 provider、具体
  model 与 request id
