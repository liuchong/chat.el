...
| 围栏代码块 | 围栏行淡化但**保留可见**（要能看出语言），块内按语言上色 | 见 Requirement 4 |
| 无序列表 `-` `*` `+` | 符号按层级换成 `•` `◦` `▪`，缓冲区文本不变 | `display` 属性 |
| 有序列表 `1.` | 数字保留，编号 face 淡化 | face |
| 嵌套列表 | 按缩进层级换符号，续行悬挂缩进对齐 | `display` + `wrap-prefix` |
| 任务列表 `- [ ]` `- [x]` | 换成 `☐` `☑` | `display` |
| 引用块 `>` | 尖括号隐藏，整块左侧留边距并加 face | `invisible` + `wrap-prefix` |
| 表格 | 按显示宽度补空格对齐，分隔行换成横线 | 见 Requirement 5 |
