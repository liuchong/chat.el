...
2. 不在核心路径抓取或解码图片，不渲染 LaTeX 数学公式，不执行任意 HTML/CSS/脚本
...
| setext 标题 | 下划线隐藏，按两级标题显示 | `invisible` + 分级 face |
...
| 图片 `![alt](url)` | 显示成紧凑资源节点，可点开，附带可供 UI 异步物化的描述 | `display` + resource property |
| 安全 HTML | `strong/em/code/kbd/mark/del/sub/sup/br/details/a/img` 映射到原生文本语义 | 白名单 + 文本属性 |
| 未知 HTML | 原样保留，整体淡化，不执行 | face |
| 数学 `$...$` | 原样保留，不处理 | — |
| 脚注引用 `[^id]` | 标记隐藏，显示为紧凑引用；原文可复制 | `invisible` + face |
...
转义管道 `\|` 与反引号代码区间内的 `|` MUST 是单元格内容，不得被当成列边界。
分词发生在行内渲染之前，但反斜杠与反引号仍留在单元格源码中，交给统一的行内层处理。
...
行内渲染 MUST 先定位下一个可能的起始符，再按该起始符分派到一个小的规则集合；
MUST NOT 在每个已命中构造之后，把全部规则重新搜索一遍整段剩余文本。后一种写法会让
一条包含大量链接或强调的长行退化为二次复杂度。
...
- **图片不在核心渲染器内联。** Markdown 与 HTML 图片都形成可点开的资源节点，并携带
  `chat-markdown-resource` 描述；UI 可以异步物化，纯渲染路径不读文件、不联网、不解码
- **数学不渲染。** 原样显示。渲染 LaTeX 需要外部进程或图片，两者都在 non-goals
- **HTML 只解释安全语义白名单。** 链接、图片和常用行内语义映射为文本属性；未知标签
  原样淡化，样式、脚本和任意 DOM 从不执行
