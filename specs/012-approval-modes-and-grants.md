...
- 审批跳过。
- 执行隔离同步解除：凡是模型可触发的执行路径——`shell_execute`、后台
  `work_task_start` / `programming_compile_task`、verification 步骤、REPL 进程——
  在 `dangerous` 下一律改用无限制的 `local` 后端（继承完整环境、真实 HOME、有网络、
  不生成 sandbox profile），见 spec 023 的修订。`manual` / `guarded` 保持各自的
  `inspect` / `build` 沙箱不变。一次性同意（人批准一次、guard 裁决一次）不解除
  隔离——只有模式本身才解除，否则"同意过一条命令"就会变成"整个会话都出了沙箱"。
...
3. `dangerous` 下：`shell_execute` 执行 `git push --dry-run` 不被闸门拒绝，
   不产生审批事件。
3a. `dangerous` 下：`shell_execute`、`work_task_start` /
   `programming_compile_task`、verification 步骤与 REPL 进程的执行请求均落在
   `local` 后端、policy 为 `local`（无沙箱、有网络、继承完整环境）；`manual` 与
   `guarded` 下同一类命令仍分别是 `inspect` / `build` policy 且无网络。
