...
Keep the shared reading workflow stable while continuing to raise coverage around file-editing reliability, update-patch EOF semantics, replace narrowing behavior, whitespace-aware context refusal behavior, and command surfaces.
...
Keep pushing test density around patch-engine and search-replace edge cases until add, update, move, count narrowing, and newline semantics are no longer the easiest path to a production failure.
