...
Keep the shared reading workflow stable while continuing to raise coverage around file-editing reliability, standard and codex newline-marker compatibility, unified-diff metadata compatibility, hunk validation, hunk-header validation, header-drift tolerance, empty-update rejection, replace success semantics, invalid-regexp diagnostics, empty-pattern validation, empty-match regexp refusal, directory-path validation, ambiguous line-hint handling, add-file validation, regexp narrowing behavior, move-only patch behavior, patch atomicity, pure insert and delete hunk compatibility, sequential hunk line-delta stability, update-patch EOF semantics, replace narrowing behavior, whitespace-aware context refusal behavior, and command surfaces.
