...
### Pre-1.0 Clean Break First

本项目尚未发布正式版、尚未上线、没有需要维护的既有用户合同。当前阶段的首要
产品原则是形成正确、清晰、直接的新设计，不为不存在的历史包袱付出复杂度。

这个原则只决定“正式设计已经变更后如何实现”，绝不授权 Agent 自行破坏或绕过
既有正式设计。尚未被明确修改的 Spec、决策和规则始终具有约束力；现有测试若在验证
这些合同，测试失败就是回归，不得被称为兼容负担。

- 修改行为前必须先定位该行为的权威 Spec、正式决策和现行规则
- 用户没有要求修改设计时，只能修正实现使其符合现有设计，不得暗中重定义合同
- 正式设计只能由一次明确、完整的正式设计修订来替换；实现、重构、测试清理和所谓
  clean break 都无权自行宣布某项尚未修订的设计过期
- 确需修改设计时，必须先把相关 Spec、规则、数据合同和交互语义整体修订为一套完整、
  自洽、无冲突的新设计，再按新设计替换实现
- 新设计必须说明被替换的正式设计范围；范围外的既有设计不得受损
- clean break 的生效边界是“已经正式完成的设计变更”，不是某段代码看起来旧或不方便
- 不得以“不兼容”为理由删除仍在验证现行设计的测试、绕过约束、降低验收标准，或顺带
  改坏本次正式设计变更范围之外的行为

- 产品内部的旧 schema、旧命令、旧配置、旧缓存、旧别名和旧调用路径默认全部不兼容
- 设计发生变化时直接替换事实来源，同时删除旧实现、旧入口、旧测试和旧文档
- 禁止新增 migration、fallback、shim、compatibility wrapper、双写、弃用期和静默格式猜测
- 禁止以“以后可能有人使用”“更稳妥”“先兼容一下”为理由保留两套语义或绕路设计
- 过期状态应明确拒绝或直接丢弃；可重建缓存直接重建，不编写迁移器
- 修改已有模块时，若发现同一能力的新旧路径并存，应在当前改动范围内收敛为唯一合同
- 测试必须验证当前唯一设计，不得把旧行为继续可用当作成功标准
- 文档和提示词只描述当前设计，不保留旧名称、旧入口或迁移说明

只有两类情况不属于这里禁止的“历史兼容”：项目明确支持的运行平台范围，例如
Emacs 27+；以及当前外部协议真实要求的版本或 wire contract。即使如此，也只实现
当前明确支持的合同，不得顺手增加猜测式兼容。任何产品历史兼容只有在用户针对一个
已存在、已发布、确有用户的数据合同明确要求时才允许，并且必须先记录理由和删除条件。

本节必须常驻上下文：它直接位于每次任务必读的 `AGENTS.md` 顶层原则中，不得仅放入
阶段日志、压缩摘要或可选参考文档，也不得被后续局部规则稀释。
...
- agent 在开始实现前，必须按以下顺序读取项目上下文：
  ...
  - 根据任务类型选择性读取：
    ...
    - 外部平台或协议支持问题优先读 `20-reference/compatibility/`
