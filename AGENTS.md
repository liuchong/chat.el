...
- 禁止使用工具或 IDE 自带的界面提问功能向用户提问
- 包括但不限于结构化提问工具 多选题弹窗 选项卡 单选列表 确认对话框 表单式问卷
- 这类控件只能显示极短的选项标签 装不下判断所需的背景 证据和推理
- 用户看到的是一串结论 看不到结论怎么来的 因此无法判断 只能瞎选或者取消
- 不得以选项太多 正文太长 这样更清晰 用户可以选 Other 为理由绕开本规则
- 不得把弹窗当作正文说明的替代品 也不得先弹窗再补充说明

### How To Ask

所有提问一律写在对话正文里 用自然语言写清楚。
每个需要用户决策的问题必须包含：

- 为什么要问：问题的来源 触发它的具体事实 以及不回答会卡住什么
- 目的是什么：这个决策会改变后续哪些动作 哪些范围 哪些验收标准
- 证据位置：文件路径和具体行号或函数名 必要时贴出关键代码片段
- 每个选项的判断标准：成立前提 代价 风险 以及在什么条件下它才是对的
- 推荐意见：推荐哪个 为什么推荐 以及什么情况下推荐会翻转
- 不确定的部分：哪些已核实并给出核实方式 哪些是推测 哪些需要外部信息才能确认
...
- 只问真正属于用户的决策：价值取向 优先级 风险偏好 业务判断 需要权限或外部信息才能回答的事
- 信息不足到用户无法判断时 先补齐信息再问 而不是把不完整的问题抛出去
- 相关的几个问题连同背景一次讲完 不要反复追问同一件事

### If Already Violated

- 已经弹过窗的 立即在正文里补齐完整信息重问 并说明上次信息不足
- 用户取消弹窗不等于拒绝决策或默认同意
- 取消应理解为信息不足无法作答 必须改用正文重新说明
